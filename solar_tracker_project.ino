...
MotorPositions performSearch(int pan_start, int tilt_start, int pan_end, int tilt_end, int tilt_increment, int pan_increment) {
  ...
  for (int tilt_pos = tilt_start; tilt_pos <= tilt_end; tilt_pos += tilt_increment) {
    ...
    while (continue_moving_pan) {
      ...
      // DAQSendDataOverSerial(pan_pos, tilt_pos, current_voltage);
      delay(1000); // delay between next move and measure
    }
...
void DAQSendDataOverSerial(int panAngle, int tiltAngle, float voltage) {
  Serial.println( (String) "DATA,DATE,TIME,TIMER," + 0 + "," +
                  0 + "," + 0 + "," + 0+"," + 0 + "," +
                  panAngle + "," + tiltAngle + "," + voltage);
}
